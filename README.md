...
- cask / VSCode 拡張機能のインストール (`homebrew/Brewfile` を post-packages hook で `brew bundle`)
...
settings.json / keybindings.json は `vscode/` 配下を実体として `[dotfiles]` で symlink 管理する（拡張機能は `homebrew/Brewfile` の `vscode` ディレクティブで管理）。
VSCode 上で設定を変更すると symlink 経由でリポジトリ実体が書き換わるため、差分を確認してコミットする。

注意: VSCode 標準の Settings Sync と併用すると二重管理になる。
dotfiles 管理に寄せるため、Settings Sync は無効化するか、`設定の同期: 構成` で Settings / Keybindings を同期対象から外すこと。
...
# cask / VSCode 拡張機能の追加 (homebrew/Brewfile に追記して)
mise bootstrap --only packages

# インストール済み VSCode 拡張機能の一覧 (Brewfile 更新時のエクスポート用)
...
├── homebrew/
│   └── Brewfile          # GUI アプリ (cask) と VSCode 拡張機能。brew bundle で適用 (理由はファイル冒頭コメント参照)
├── scripts/
...
├── vscode/               # VSCode ユーザー設定 (settings/keybindings。User 配下へ個別 symlink)
