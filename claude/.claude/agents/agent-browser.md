...
1. **既存セッションをクリア**してからプロファイル付きで起動する（`--profile` は起動時にしか適用されないため）
  ...
  agent-browser --profile develop open URL
  ```
2. ページ読み込み後にスクリーンショットで認証状態を確認する
3. **ログイン画面が表示された場合**: 認証プロファイルが未作成または期限切れ。ユーザーに手動ログインを依頼して終了する
  - ユーザーは `agent-browser --profile develop --headed open URL` で直接ブラウザを開き、手動でログインする
...
| 認証フロー（Firebase） | `--profile develop` で起動 → 認証状態が自動復元される → ログイン画面が出たらユーザーに手動ログインを依頼 |
